...
public enum PhotosManager {
    ...
    public static func allPhotoCount() -> Int {
        var sendCount = 0
        var sendVideoCount = 0

        let fetchOptions = PHFetchOptions()
        fetchOptions.sortDescriptors = [NSSortDescriptor(key: "creationDate", ascending: false)]

        // アルバムをフェッチ
        let assetCollections = PHAssetCollection.fetchAssetCollections(
            with: .smartAlbum,
        ...
        assetCollections.enumerateObjects { assetCollection, index, _ in
            ...
            print(
                "[PhotosManager]",
                assetCollection.localizedTitle ?? "nil",
                PHAsset.fetchAssets(in: assetCollection, options: nil).count
            )
        ...
        // Videosをフェッチ
        let assetVideoCollections = PHAssetCollection.fetchAssetCollections(
            with: .smartAlbum,
        ...
        assetVideoCollections.enumerateObjects { assetCollection, index, _ in
            ...
            // アルバムタイトル
            // print(assetCollection.localizedTitle ?? "")
            // print("[PhotosManager] video ",assetCollection.localizedTitle ?? "nil", PHAsset.fetchAssets(in:
            // assetCollection, options: nil).count)
    ...
    public static func favoriteThumbnail(albumInfo: AlbumInfo, partsCount: Int, thumbnailSize: CGSize) -> [UIImage] {
        ...
        let requestFetchResult = PHAssetCollection.fetchAssetCollections(
            with: .smartAlbum,
            subtype: .smartAlbumFavorites,
            options: nil
        )
